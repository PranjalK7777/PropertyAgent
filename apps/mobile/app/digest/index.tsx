...
export default function DigestHistoryScreen() {
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Digest History', headerBackTitle: 'Settings' }} />
      <ScrollView
                ...
                >
                  ...
                  <View className="flex-row items-center justify-between px-4 py-4">
                    ...
                    <View className="flex-row items-center gap-3">
                      <StatBadge value={digest.stats.totalConversations} label="chats" color="blue" />
                      <StatBadge value={digest.stats.hotLeads} label="hot" color="red" />
                      <Ionicons
                  ...
                  {isExpanded && (
                    <View className="border-t border-gray-100 px-4 py-4">
                      <View className="flex-row flex-wrap gap-3 mb-4">
                        <StatCard value={digest.stats.totalConversations} label="Total Chats" icon="chatbubbles-outline" />
                        <StatCard value={digest.stats.newToday} label="New Today" icon="person-add-outline" />
                        <StatCard value={digest.stats.hotLeads} label="Hot Leads" icon="flame-outline" />
                        <StatCard value={digest.stats.escalations} label="Escalations" icon="alert-circle-outline" />
                      </View>
...
function StatBadge({ value, label, color }: { value: number; label: string; color: 'blue' | 'red' }) {
  const colorClass = color === 'blue' ? 'bg-blue-50 text-blue-700' : 'bg-red-50 text-red-700';
  return (
    <View className={`px-2 py-1 rounded-md ${colorClass}`}>
      <Text className={`text-xs font-bold ${colorClass.split(' ')[1]}`}>{value} {label}</Text>
    </View>
  );
}
...
function StatCard({ value, label, icon }: { value: number; label: string; icon: any }) {
  return (
    <View className="flex-1 min-w-[40%] bg-gray-50 rounded-lg p-3">
